...
public class MinLength<L> : IConstraint<string>
    where L : INat
{
    public static MinLength<L> Instance { get; } = new ();
    private static readonly ErrorList Error = ConstraintErrors.Current.MinLength("String", L.Value).ToList();
    ...
    where L : INat
{
        ...
        where T : IReadOnlyCollection<E>
    {
        if (item.Count < L.Value)
            return ConstraintErrors.Current.MinLength(type, L.Value).ToList();
        else
            return item;
    }
